...
### docker nacos-mysql单机版安装

```bash
docker pull nacos/nacos-server:2.0.3


# 先启动一个nocas，稍后将配置文件拷贝出来进行挂载
...
# 将刚才从容器中拷贝的配置文件移动到指定目录下
cp -r conf/* /docker/nacos/conf

# 修改数据库连接，要先建好mysql数据库
cd /docker/nacos/conf
vim application.properties 
# spring.datasource.platform=mysql
# db.num=${MYSQL_DATABASE_NUM:1}
# db.url.0=jdbc:mysql://127.0.0.1:3306/nacos?serverTimezone=Asia/Shanghai&useUnicode=true&characterEncoding=UTF-8&autoReconnect=true&useSSL=false
# db.user=root
# db.password=123456
...
# 重新启动，启动前需要在mysql中执行相应的sql，sql在下面
...
--name nacos -d nacos/nacos-server:2.0.3
