...
#### 配置Sharding Proxy
在 `/${your_work_dir}/conf/` 创建 `server.yaml` 和 `config-xxx.yaml` 文件，进行服务器和分片规则配置，配置详见以下 `配置规则`
...
-e PORT=3308 -p13308:3308 apache/sharding-proxy:latest
```


#### 配置规则

`Sharding-Proxy` 支持多逻辑数据源，每个以 `config-` 前缀命名的yaml配置文件，即为一个逻辑数据源，以下是 `config-xxx.yaml` 的配置配置示例
...
用于执行登录Sharding Proxy的权限验，配置用户名、密码、可访问的数据库后，必须使用正确的用户名、密码才可登录Proxy
