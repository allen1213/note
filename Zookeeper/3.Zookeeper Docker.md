...
#创建挂载目录
mkdir -p /dockr/zk/conf
mkdir -p /docker/zk/data
mkdir -p /docker/zk/log

#启动容器
docker run -d --name zk --restart always zookeeper

#将配置文件拷贝出来
docker cp -a zk:/conf/zoo.cfg /docker/zk/conf/zoo.cfg
```
...
-v /dockr/zk/conf/zoo.cfg:/conf/zoo.cfg \
-v /dockr/zk/data:/data \
-v /dockr/zk/log:/datalog \
-d zookeeper
```
...
配置
```shell
mkdir -p /usr/local/zookeeper-cluster/node4
mkdir -p /usr/local/zookeeper-cluster/node5
mkdir -p /usr/local/zookeeper-cluster/node6
